...
class MainActivity : AppCompatActivity() {
    ...
    private fun showCategory(category: String) {
        ...
        binding.categoryPermissions.visibility =
            if (category == "permissions") View.VISIBLE else View.GONE
        binding.categoryActions.visibility = if (category == "actions") View.VISIBLE else View.GONE
        binding.categoryServices.visibility =
            if (category == "services") View.VISIBLE else View.GONE
    ...
    private fun showAddActionDialog(service: Service) {
        MaterialAlertDialogBuilder(this)
            .setTitle("Add Action")
            .setItems(
                arrayOf("Tap", "Get Info", "HTTP Request", "AI Action", "Delay", "Run Service")
            ) { _, which ->
                when (which) {
    ...
    private fun addGetInfoAction(service: Service) {
        ...
        MaterialAlertDialogBuilder(this)
            ...
            .setItems(types) { _, which ->
                ServiceRepository.addActionToService(
                    service.id,
                    Action.GetInfoAction(infoType = InfoType.values()[which]),
                )
                refreshServices()
            }
            .setNegativeButton("Cancel", null)
    ...
    private fun showAddTriggerDialog(service: Service) {
        MaterialAlertDialogBuilder(this)
            ...
            .setItems(
                arrayOf("Log Trigger", "Time Trigger", "Accessibility Trigger")
            ) { _, which ->
                when (which) {
    ...
    private fun toggleBridge() {
        ...
        LogBuffer.i("CWBridge", "bridge stopped")
            ...
            if (!logcatReader.hasPermission()) {
                LogBuffer.w(
                    "CWBridge",
                    "without READ_LOGS, only in-app test invokes work — grant via ADB",
                )
            }
        }
        refreshUi()
    ...
    private fun refreshUi() {
        ...
        binding.logcatState.text =
            if (logcatReader.hasPermission()) "logcat OK" else "logcat needs grant"
